...
#define tempset_topic mqtt_topic "/tempset" //not working, still in test
#define onoff_topic mqtt_topic "/onoff"
#define ambtemp_topic mqtt_topic "/ambtemp" //reused as external puffer probe as Klover idro don't provide ambtemp (as i know)
...
#define waterpres_topic mqtt_topic "/waterpres" //not working on Klover Stoves, still in test
...
#define ambTempAddr 0x4d //external puffer probe
#define tempSetAddr 0x7D
#define fumesTempAddr 0x5A
#define flamePowerAddr 0x34
#define waterTempAddr 0x03
#define waterSetAddr 0x51 //as i know this hex value is used only in klover idro stove, the value is increased by 5
#define waterPresAddr 0x3C // hex don't working, still in test on klover stove
...
void reconnect() //Connect to MQTT server
{
    ...
    if (client.connected())
    {
      ...
      String flame_power_sensor_payload = "{\"name\": \"Flame Power\", \"state_topic\": \"" flame_topic "\", \"qos\": 0, \"icon\": \"mdi:fire\", \"unique_id\": \"c3aecb86-66e2-4358-bccb-e3b620f3d28b\",\"device\": " device_information "}";
      ...
      if (hydro_mode == 1)
      {
        ...
        String water_set_sensor_topic = "homeassistant/sensor/Micronova/WaterSet/config";
        String water_set_sensor_payload = "{\"name\": \"Water Set Temperature\", \"state_topic\": \"" waterset_topic "\", \"qos\": 0, \"device_class\": \"temperature\", \"state_class\": \"measurement\", \"unit_of_measurement\": \"°C\", \"icon\": \"mdi:thermometer\", \"unique_id\": \"2e272bb3-7b55-4867-bf2a-b8772d0ae34d\", \"device\": " device_information "}";
        client.publish(water_set_sensor_topic.c_str(), water_set_sensor_payload.c_str(), true);
      }
    ...
    if (rxCount == 2)
    {
        ...
        case stoveStateAddr:
            ...
            case 1:
                ...
                break;
            case 2:
                client.publish(state_topic, "Carico Pellet", true);
                delay(1000);
                client.publish(onoff_topic, "ON", true);
                break;
            case 3:
                client.publish(state_topic, "Fuoco Presente", true);
                delay(1000);
                client.publish(onoff_topic, "ON", true);
                break;
            case 4:
                client.publish(state_topic, "Lavoro", true);
                delay(1000);
                client.publish(onoff_topic, "ON", true);
                break;
            case 5:
            ...
            case 7:
                ...
                break;
            case 8:
                client.publish(state_topic, "Mancanza Pellet!", true);
                break;
            case 9:
                ...
                break;
            case 10:
                client.publish(state_topic, "In Allarme!", true);
                break;
            }
...
void loop()
{
    ...
    if (currentMillis - previousMillis >= 25000)
    {
        ...
        getStates();
        client.publish(pong_topic, "Connesso");
    }
    if (deepSleep == 1)   //Does not work without hardware modification (a cable must be connected between RST and D0)
    {
